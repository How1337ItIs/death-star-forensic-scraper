...
class AdvancedCaptureResult:
    ...
    # Structured data extraction (Next.js, Nuxt, GraphQL, etc.)
    next_data: Optional[Dict] = None  # Parsed __NEXT_DATA__ JSON
    nuxt_data: Optional[Dict] = None  # Parsed __NUXT__ data
    graphql_responses: List[Dict] = field(default_factory=list)  # Captured GraphQL responses
    apollo_state: Optional[Dict] = None  # __APOLLO_STATE__ cache
    redux_state: Optional[Dict] = None  # Redux store state
    remix_context: Optional[Dict] = None  # Remix framework context
    gatsby_data: Optional[Dict] = None  # Gatsby page data
    initial_data: Optional[Dict] = None  # Generic __initialData__ or similar
...
class AdvancedCapture:
    ...
    async def capture_advanced(
        ...
        # Extract structured data (__NEXT_DATA__, __NUXT__)
        next_data = self._extract_next_data(html)
        nuxt_data = self._extract_nuxt_data(html)

        # Extract framework state (Apollo, Redux, Remix, Gatsby, etc.)
        framework_state = await self._extract_framework_state(page, html)

        # Capture GraphQL responses
        graphql_responses = await self._capture_graphql_responses(page)
        ...
        if next_data:
            logger.info(f"Extracted __NEXT_DATA__ ({len(json.dumps(next_data))} bytes)")
        if nuxt_data:
            logger.info(f"Extracted __NUXT__ data ({len(json.dumps(nuxt_data))} bytes)")
        if graphql_responses:
            logger.info(f"Captured {len(graphql_responses)} GraphQL responses")
        ...
        return AdvancedCaptureResult(
            ...
            apollo_state=framework_state.get("apollo_state"),
            redux_state=framework_state.get("redux_state"),
            remix_context=framework_state.get("remix_context"),
            gatsby_data=framework_state.get("gatsby_data"),
            initial_data=framework_state.get("initial_data"),
        )
    ...
    # ------------------------------------------------------------------
    # Structured data extraction (__NEXT_DATA__, __NUXT__, GraphQL)
    # ------------------------------------------------------------------
    ...
    def _extract_next_data(self, html: str) -> Optional[Dict]:
        ...
        Next.js embeds server-rendered data as JSON inside:
          <script id="__NEXT_DATA__" type="application/json">{...}</script>

        This contains page props, build info, route data, and API responses
        that the page pre-fetched during SSR. For e-commerce sites like Walmart,
        this often includes product data, order history, user info, etc.
        """
        match = re.search(
    ...
    async def _extract_framework_state(self, page, html: str) -> Dict[str, Optional[Dict]]:
        """Extract state from various JS frameworks (Apollo, Redux, Remix, Gatsby, etc.)."""
        state = {
            "apollo_state": None,
            "redux_state": None,
            "remix_context": None,
        ...
        # Apollo Client cache (__APOLLO_STATE__)
        try:
            apollo = await page.evaluate('() => window.__APOLLO_STATE__ || null')
            if apollo:
                state["apollo_state"] = apollo
                logger.info(f"Extracted __APOLLO_STATE__ ({len(json.dumps(apollo))} bytes)")
        except Exception as e:
            logger.debug(f"Apollo state extraction failed: {e}")

        # Redux store state
        try:
            redux = await page.evaluate('''() => {
            ...
            if redux:
                state["redux_state"] = redux
                logger.info(f"Extracted Redux state ({len(json.dumps(redux))} bytes)")
        except Exception as e:
            logger.debug(f"Redux state extraction failed: {e}")

        # Remix context
        try:
            remix = await page.evaluate('() => window.__remixContext || null')
            if remix:
                state["remix_context"] = remix
                logger.info(f"Extracted __remixContext ({len(json.dumps(remix))} bytes)")
        except Exception as e:
            logger.debug(f"Remix context extraction failed: {e}")

        # Gatsby page data
        try:
            gatsby = await page.evaluate('''() => {
                if (window.___GATSBY_DATA___) return window.___GATSBY_DATA___;
                // Also check for page-data embedded in script tags
                const el = document.getElementById('gatsby-chunk-mapping');
                if (el) return JSON.parse(el.textContent);
                return null;
            }''')
            if gatsby:
                state["gatsby_data"] = gatsby
                logger.info(f"Extracted Gatsby data ({len(json.dumps(gatsby))} bytes)")
        except Exception as e:
            logger.debug(f"Gatsby data extraction failed: {e}")

        # Generic initial data patterns
        try:
            initial = await page.evaluate('''() => {
                const candidates = [
                    '__initialData__', '__INITIAL_DATA__', '__APP_DATA__',
                    '__PAGE_DATA__', '__SERVER_DATA__', '__SSR_DATA__',
                    '__UNIVERSAL_DATA__',
                ];
            ...
            if initial:
                ...
                logger.info(f"Extracted {initial.get('key', 'unknown')} data")
        except Exception as e:
            logger.debug(f"Initial data extraction failed: {e}")

        return state

    async def _capture_graphql_responses(self, page) -> List[Dict]:
        """
        Extract GraphQL responses that were already captured by network listeners,
        and also scan the DOM for any inline GraphQL data (Apollo cache, etc.).
        """
        captured = []

        # Try to extract Apollo Client cache from window.__APOLLO_STATE__
        try:
            apollo_state = await page.evaluate('''() => {
                if (window.__APOLLO_STATE__) return window.__APOLLO_STATE__;
                // Also check Next.js Apollo pattern
                const nd = window.__NEXT_DATA__;
                if (nd && nd.props && nd.props.apolloState) return nd.props.apolloState;
                return null;
            }''')
            if apollo_state:
                captured.append({
            ...
            logger.debug(f"Apollo cache extraction failed: {e}")

        # Try to extract any window.__RELAY_STORE__ (Facebook Relay)
        try:
            relay_store = await page.evaluate('''() => {
                return window.__RELAY_STORE__ || null;
            }''')
            if relay_store:
                captured.append({
                    "source": "relay_store",
            ...
            logger.debug(f"Relay store extraction failed: {e}")
